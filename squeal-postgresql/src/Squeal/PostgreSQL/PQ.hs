...
instance (Monad m, Semigroup r, schemas0 ~ schemas1) => Semigroup (PQ schemas0 schemas1 m r) where
  f <> g = pqAp (fmap (<>) f) g

instance (Monad m, Monoid r, schemas0 ~ schemas1) => Monoid (PQ schemas0 schemas1 m r) where
  mempty = pure mempty
