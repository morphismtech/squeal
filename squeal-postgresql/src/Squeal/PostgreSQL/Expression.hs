...
-}
class (PGTyped (PGTypeOf ty), KnownNat n)
...
instance {-# OVERLAPPING #-} KnownSymbol column
  => HasColumn column ((column ::: optionality ty) ': tys) '( '[], ty)
...
def
  ...
  => Expression '[] 'Ungrouped params '(constraints, (nullity ty))
...
unDef
  :: 'Default `In` constraints
  => Expression '[] 'Ungrouped params '( '[], (nullity ty))
  -- ^ not @DEFAULT@
  -> Expression '[] 'Ungrouped params '(constraints, (nullity ty))
...
serial2, smallserial :: TypeExpression '(constraints, ('NotNull 'PGint2))
...
serial4, serial :: TypeExpression '(constraints, ('NotNull 'PGint4))
serial4 = UnsafeTypeExpression "serial4"
serial = UnsafeTypeExpression "serial"
-- | not a true type, but merely a notational convenience for creating
-- unique identifier columns with type `'PGint8`
serial8, bigserial :: TypeExpression '(constraints, ('NotNull 'PGint8))
