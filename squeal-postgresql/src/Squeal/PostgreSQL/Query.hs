...
selectDistinct
  ...
  => Selection outer commons 'Ungrouped db params from columns
  -- ^ selection
...
selectDistinct_
  :: (SListI columns, columns ~ (col ': cols))
  => NP (Aliased (Expression outer commons 'Ungrouped db params from)) columns
  -- ^ select list
  -> TableExpression outer commons 'Ungrouped db params from
  -- ^ intermediate virtual table
  -> Query outer commons db params columns
...
selectDistinctOn
  :: (SListI columns, columns ~ (col ': cols))
  => [SortExpression outer commons 'Ungrouped db params from]
